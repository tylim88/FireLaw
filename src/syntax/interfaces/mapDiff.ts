...
```
 */
export type MapDiff<T extends MetaType['read'] | MetaType['write']> = {
	/**
	 * @returns Returns a rules.Set, which lists any keys that the Map calling diff() contains that the Map passed to diff() does not.
	 * ```ts
	...
	/**
	 * @returns Returns a rules.Set, which lists any keys that have been added to, removed from or modified from the Map calling diff() compared to the Map passed to diff(). This function returns the set equivalent to the combined results of MapDiff.addedKeys(), MapDiff.removedKeys() and MapDiff.changedKeys().
	 * ```ts
	...
	/**
	 * @returns Returns a rules.Set, which lists any keys that appear in both the Map calling diff() and the Map passed to diff(), but whose values are not equal.
	 * ```ts
	...
	/**
	 * @returns Returns a rules.Set, which lists any keys that the Map calling diff() does not contain compared to the Map passed to diff().
	 * ```ts
